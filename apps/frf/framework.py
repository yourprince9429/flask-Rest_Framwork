...
def request_wrapper(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        return self.dispatch_after_request(result)
...
class APIView(MethodView):
    ...
    def get_query_set(self):
        ...
        if self.query_set is not None:
            ...
            query_set = self.serializer.model_class.query.filter_by(**self.query_set)
        ...
        # )
            ...
            query_set = self.serializer.model_class.query
    ...
    def _filter_queryset(self):
        ...
        if request.method.lower() == "get":
            if self.look_up is None:
                ...
                query_look_up = {k: None if v == "null" else v for k, v in query_look_up.items()}
    ...
    def _special_filter_queryset(self, qs):
        ...
        for i in self.special_look_up:
            lt = self.request_data.get(i + "_lt")
            gt = self.request_data.get(i + "_gt")
            attr = getattr(self.serializer.model_class, i, None)
            if lt:
                lt = datetime.fromtimestamp(float(lt))
                qs = qs.filter(attr <= lt)
            if gt:
                gt = datetime.fromtimestamp(float(gt))
                qs = qs.filter(attr >= gt)
        return qs

    # def _method_mix(self, qs, lk):
    #     # 小于，大于，不等于，in列表
    #     meth_list = ["<", ">", "!", "["]
    #     method = self.request_data.get(lk)[0:1]
    #     if method not in meth_list:
    #         return qs
    #
    #     value = self.request_data.get(lk)[1:]
    #     model_class = self.serializer.model_class
    #     attr = getattr(model_class, lk, None)
    #     if not attr:
    #         raise SQLAlchemyError("%s has no field %s" % ("table", lk))
    #
    #     if method == "<":
    ...
    def dispatch_request(self, *args, **kwargs):
        ...
        self.meth_name = request.method.lower()
        if request.method.lower() == "get":
            if self.pk_field in request.args:
                meth = getattr(self, 'retrieve', None)
                self.meth_name = "retrieve"
                print(meth)
